...
int main( int argc, char** argv )
{
	...
	for (int step =1; step<=2; step++)
	{
		...
		{
			Mat temp=img;
			drawMSERs(temp,&regions);
			// imshow("mser",temp);
			char tempfile[1000];
			sprintf(tempfile,"%s.%d.mser.jpg",argv[1],step);
			imwrite(tempfile,temp);
		}
		...
		for (int i=regions.size()-1; i>=0; i--)
		{
			...
			if ( (regions.at(i).stroke_std_/regions.at(i).stroke_mean_ > 0.8) || 
				(regions.at(i).num_holes_>3) || (regions.at(i).bbox_.width <=4) || 
		...
		*/
		...
		for (int f=0; f<NUM_FEATURES; f++)
		{
			...
			//the meaningful_clusters generated by feature f
			{
				...
				// cout<<"feature "<<f<<" based clustering:"<<endl;
				// imshow("tmp_segmentation..",tmp_segmentation);
				// imshow("all_segmentation..",all_segmentations);
				// waitKey(0);
			}
		...
		}
		{
			...
			sprintf(str,"%s.%d.meaningfulcluster.jpg",argv[1],step);
		...
		*/
		...
		//@去重
		{
...

			{
				...
				sprintf(str,"%s.%d.contour.jpg",argv[1],step);
				imwrite(str,tmp);
				*/
				...
				tmp=img;
				drawRects(tmp,lines);
				sprintf(str,"%s.%d.lines.jpg",argv[1],step);
				// imshow(str,tmp);
				imwrite(str,tmp);
				waitKey(0);

				// cout<<"coutContours:"<<endl;
				if(step==1) cout<<"<image>"<<endl<<"<imageName>"<<argv[1]<<"</imageName>"<<endl
				...
				if(step==2) cout<<"</taggedRectangles>"<<endl<<"</image>"<<endl;
			}
